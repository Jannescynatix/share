...
// Funktion zum Anzeigen von Pop-ups (für Admin-Panel)
...
function displayRoomDetails(room) {
    ...
    deleteButton.addEventListener('click', () => {
        // Die Bestätigung wird beibehalten, da es sich um eine zerstörerische Aktion handelt
        if (confirm(`Möchtest du den Raum "${room.roomName}" wirklich löschen?`)) {
            socket.emit('admin:delete-room', room.roomName);
            showPopup(`Raum "${room.roomName}" wurde gelöscht.`);
        }
    });
    ...
    room.chatMessages.forEach(msg => {
        ...
        deleteBtn.addEventListener('click', () => {
            ...
            showPopup(`Nachricht von ${msg.senderName} wurde gelöscht.`);
        });
    ...
    room.users.forEach(user => {
        ...
        userItem.innerHTML = `<div><b>Name:</b> ${user.name} ${user.id === room.owner ? '(Ersteller)' : ''}<div class="user-details">IP: ${user.ip || 'Unbekannt'}<br>Gerät: ${user.device || 'Unbekannt'}<br>Browser: ${user.browser || 'Unbekannt'}</div></div>`;
        ...
        if (user.id !== room.owner) {
            ...
            kickBtn.addEventListener('click', () => {
                ...
                showPopup(`Benutzer "${user.name}" wurde gekickt.`);
            });
