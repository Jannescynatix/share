...
function displayRoomDetails(room) {
    ...
    textHeader.textContent = 'Editor-Text';
    textPanel.appendChild(textHeader);

    // NEU: Seitenverwaltung im Admin-Panel
    const pageSelectContainer = document.createElement('div');
    pageSelectContainer.style.display = 'flex';
    pageSelectContainer.style.alignItems = 'center';
    pageSelectContainer.style.gap = '10px';
    const pageSelect = document.createElement('select');
    pageSelect.style.padding = '5px';
    pageSelect.style.borderRadius = '5px';
    pageSelect.style.border = '1px solid #555';
    pageSelect.style.backgroundColor = '#333';
    pageSelect.style.color = '#eee';

    const pages = Object.keys(room.pages);
    ...
    editorText.textContent = room.pages[pages[0]]; // Standardmäßig erste Seite anzeigen

    pageSelect.addEventListener('change', (event) => {
        const selectedPageName = event.target.value;
        editorText.textContent = room.pages[selectedPageName];
    });

    const deletePageBtn = document.createElement('button');
    deletePageBtn.textContent = 'Seite löschen';
    deletePageBtn.classList.add('btn', 'btn-danger', 'btn-small');
    deletePageBtn.addEventListener('click', () => {
        const selectedPage = pageSelect.value;
        if (confirm(`Möchtest du die Seite "${selectedPage}" wirklich löschen?`)) {
            socket.emit('admin:delete-page', { roomName: room.roomName, pageName: selectedPage });
        }
    });

    pageSelectContainer.appendChild(pageSelect);
    pageSelectContainer.appendChild(deletePageBtn);
    textPanel.appendChild(pageSelectContainer);
